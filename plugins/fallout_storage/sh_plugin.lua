local PLUGIN = PLUGIN
PLUGIN.name = "Storage"
PLUGIN.author = "Chessnut, SuperMicronde(modifications)"
...
properties.Add( "ns_makestorage", {
	MenuLabel = "Change to storage",
	Order = 430,
	...
	Filter = function( self, ent, ply ) -- A function that determines whether an entity is valid for this property
		...
		if ( ent:GetClass() == "nut_storage" ) then return false end
		if ( !ply:getChar():hasFlags("e") ) then return end
		if ( hook.Run("CanPlayerSpawnStorage", self, model, ent) == false ) then return false end
		if ( !STORAGE_DEFINITIONS[model:lower()] ) then return end
	...
	Receive = function( self, length, player ) -- The action to perform upon using the property ( Serverside )
		...
		storage:SetPos(ent:GetPos())
		storage:SetAngles(ent:GetAngles())
		storage:Spawn()
...
local AUTOCHANGE_CVAR = AUTOCHANGE_CVAR or CreateClientConVar("nut_autochangetostorage", "0", true, true)

if (CLIENT) then
	hook.Add("SetupQuickMenu", "addStorageTab", function(menu)
		local button = menu:addCheck(L"autoChangeToStr", function(panel, state)
			if (state) then
				RunConsoleCommand("nut_autochangetostorage", "1")
			else
				RunConsoleCommand("nut_autochangetostorage", "0")
			end
		end, AUTOCHANGE_CVAR:GetBool())
...
if (SERVER) then
	function PLUGIN:PlayerSpawnedProp( ply, model, ent )
		if ply:GetInfoNum("nut_autochangetostorage", 0) == 1 then
			local data = STORAGE_DEFINITIONS[model:lower()]
